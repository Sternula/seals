...
# Homogenize type labels.
type.key <- c( "Dead" = "Dead", "Live" = "live capture", "Live" = "Live capture", "Live" = "Live captured", "Live" = "Live-capture", "Subsistence harvest" = "Subsistence harvest" )
serology$Type <- plyr::mapvalues( x = serology$Type, 
                                  from = type.key, 
                                  to = names( type.key ) )
...
pdv.table <- serology %>%                                                                                  # From the serology data,
  ...
  mutate( Type = plyr::mapvalues( Type, "Subsistence harvest", "Live" ) ) %>%                              # Group the live and harvested samples together.
  ...
  spread( key = Type,                                                                                      # Transition to wider format.
          value = NTested ) %>% 
  ungroup() %>% 
...
cdv.table <- serology %>% 
  filter( CDV_Toxic != 1,
          !is.na( CDV_Titer ) ) %>% 
  mutate( Type = plyr::mapvalues( Type, "Subsistence harvest", "Live" ) ) %>% 
  group_by( Group, 
            Type ) %>% 
  summarize( NTested = n() ) %>% 
  spread( key = Type, 
